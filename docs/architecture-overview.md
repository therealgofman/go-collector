...
Для больших инвентарей (десятки/сотни тысяч устройств) батчевый режим реализован в `internal/collector/service.go` (`Service.Run`), а `main` только парсит CLI-флаги и делегирует выполнение:
